...
Подробные примеры запросов. Актуальные схемы также доступны в `/docs` запущенного приложения.
...
При необходимости создать `.env` и задать переменные ниже. Файла `.env.example` в репозитории нет:
